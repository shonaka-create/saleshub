...
| データ連携 | CSV/TSV貼り付け取込 + データセット閲覧。GA4→MCP連携の受け皿 (Dataset/DataRowテーブル) |
...
`DataSource` / `Dataset` / `DataRow` が受け皿。MCP エージェントが GA4 から取得したデータを
`Dataset` (列定義JSON) + `DataRow` (行JSON) に書き込めば、データ連携画面でそのまま表として閲覧できる。
